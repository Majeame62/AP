...
<body>
	...
	<div class="row">
		...
		<div class="col-md-6 well">
			...
			<a href="create_tache.php" class="btn btn-success" style="margin-left:10px;margin-top:10px;"><span class="glyphicon glyphicon-check">Créer une tâche</span></a>
			<?php 
		
			
			if($data["role"] == "Administrateur" || $data["role"] == "Responsable") {
			?>
		<form style='margin-top:10px;margin-left:10px' method="post" action="export_pdf.php">

  		<button type="submit" name="export_pdf" class="btn btn-primary">telecharger les taches en cours en PDF</button>
		</form> <?php }?>
			<br />
				...
				<tbody>
					...
					while($fetch = $query->fetch(PDO::FETCH_ASSOC)){
						while($fetch2 = $query2->fetch(PDO::FETCH_ASSOC)){
							...
							$query3 = $bdd->prepare("SELECT * FROM utilisateurs WHERE id = ?");
					 $query3->execute([$fetch2['id_utilisateurs']]);
					 $user = $query3->fetch(PDO::FETCH_ASSOC);?>
							<tr>
								...
								<td><?php  echo $fetch2['priorité']?></td>
								<td colspan="2">
									<center>
										<?php
